...
interface RegistrationFormProps {
  ...
  onPreFormSuccess: (data: FormData) => Promise<void>;
}

const RegistrationForm: FC<RegistrationFormProps> = ({ onRegister, onPreFormSuccess }) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (validate()) {
      try {
        ...
        await onPreFormSuccess(formData);
        onRegister({
          name: formData.name,
...
const FilterDropdown: FC<FilterDropdownProps> = ({
  ...
  <div>
    ...
    <select
      ...
      value={value}
      onChange={onChange}
      className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-[#2691e4] focus:border-[#2691e4] sm:text-sm rounded-md text-gray-900"
    >
      {placeholder && (
...
const NeetPgPredictorPage: FC = () => {
  ...
  const handlePreFormSuccess = async (data: FormData) => {
    try {
      // We're not using the response, but we need to await it
      await authenticatedStrapiInstance.post(
        "/api/bhatia-neet-pgs",
        {
          data: data,
        }
      );
      // Successfully submitted to Strapi
    } catch (error) {
  ...
  return (
    ...
    md:pt-[4rem]">
      {user ? (
        ...
        <RegistrationForm onRegister={setUser} onPreFormSuccess={handlePreFormSuccess} />
      )}
    </div>
  );
};
